...
export class ProductsDatabase extends Db {
    ...
    public listProducts = async (): Promise<Array<ProductsDB>> => {
        const products: Array<ProductsDB> = await Db.connection(ProductsDatabase.TABLE);
        return products;
    }

    public findProductById = async (id: string): Promise<ProductsDB | undefined> => {
        const [products]: Array<ProductsDB> = await Db.connection(ProductsDatabase.TABLE).select().where({ id });
        return products;
    }
    ...
    public updateProduct = async (id: string, input: UpdateProductsDB): Promise<void> => {
