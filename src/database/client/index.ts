...
import {
  ...
  PaymentCardsDB,
  updateClientDB,
  InsertAdressClientDB,
  UpdateAdressClientDB,
  GetAllAdress,
  InsertPaymentDB,
} from "../../dto/client/db";
import { Db } from "../db";
...
export class ClientDB extends Db {
  ...
  public findClientById = async (
    id: string,
  ): Promise<Client_TableDB | undefined> => {
    const [user]: Array<Client_TableDB> = await Db.connection(
      ClientDB.TABLE_CLIENT,
    )
      .select("*")
      .where({ id });
    return user;
  ...
  public findClientCompleteById = async (
    id: string,
    ...
    const [user]: Array<Client_TableDB> = await Db.connection(
      ...
      .select("*")
      .where({ id });
    const address: Array<AddressDB> = await Db.connection(
      ClientDB.TABLE_ADDRESS,
    )
      .select("*")
      .where({ client_id: id });
    const payment: Array<PaymentCardsDB> = await Db.connection(
      ClientDB.TABLE_PAYMENT,
    )
      .select("*")
      .where({ client_id: id });
  ...
  public findClientByEmail = async (
    email: string,
  ): Promise<Client_TableDB | undefined> => {
    const [user]: Client_TableDB[] | undefined = await Db.connection(
      ClientDB.TABLE_CLIENT,
    ).where({ email });
    return user;
  };
  ...
  // ADRESS
  public listAdress = async (
    client_id: string,
  ): Promise<Array<GetAllAdress>> => {
    const address: Array<GetAllAdress> = await Db.connection(
      ClientDB.TABLE_ADDRESS,
    ).where({
      client_id,
    });
  ...
  public findAdressById = async (
    id: string,
  ): Promise<Array<GetAllAdress | undefined>> => {
    const adress: Array<GetAllAdress | undefined> = await Db.connection(
      ClientDB.TABLE_ADDRESS,
    )
      .select("*")
      .where({ id });
  ...
  public findAdressByStreet = async (
    street: string,
  ): Promise<Array<GetAllAdress | undefined>> => {
    const adress: Array<GetAllAdress | undefined> = await Db.connection(
      ClientDB.TABLE_ADDRESS,
  ...
  public findPaymentMethodByNumberCard = async (
    number_card: number,
  ): Promise<GetPaymentClientDB | undefined> => {
    const [paymentByNumberCard]: Array<GetPaymentClientDB> =
      await Db.connection(ClientDB.TABLE_PAYMENT)
        .select()
        .where({ number_card });
  ...
  public findPaymentMethodById = async (
    id: string,
  ): Promise<GetPaymentClientDB | undefined> => {
    const [paymentById]: Array<GetPaymentClientDB> = await Db.connection(
      ClientDB.TABLE_PAYMENT,
    )
      .select()
      .where({ id });
