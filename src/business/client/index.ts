...
import {
  AddresClient,
  DeleteAdressClientInputDTO,
  DeleteAdressClientOutputDTO,
  DeletePaymentInputDTO,
  DeletePaymentOutputDTO,
  FindClientInputDTO,
  GetClientOutPutDTO,
  GetPaymentClient,
  InsertAdressClientInputDTO,
  InsertAdressClientOutputDTO,
  InsertPaymentInputDTO,
  InsertPaymentOutputDTO,
  LoginClientInputDTO,
  LoginClientOutputDTO,
  SignupClientInputDTO,
  SignupClientOutputDTO,
  UpdateAdressClientInputDTO,
  UpdateAdressClientOutputDTO,
  deleteClientInputDTO,
  deleteClientOutputDTO,
  updateClient,
...
export class ClientBusiness {
  ...
  public signup = async (
    ...
    if (exists || cpfExists) {
      throw new BadRequest(
        "Desculpe, verifique os dados informados e tente novamente!",
      );
    }
  ...
  public login = async (
    input: LoginClientInputDTO,
  ): Promise<LoginClientOutputDTO> => {
    const { email, password } = input;
    const exists = await this.clientDb.findClientByEmail(email);

    if (!exists) {
      throw new BadRequest(
        "'cliente' - não encontrado - sugestão faça o signup",
      );
    }
    const hashIsValid = await this.hashManager.compare(
      password,
      exists.password,
    );
  ...
  public findClient = async (
    ...
    const address: Array<AddresClient> = exists.address.map((endereco) =>
      new AddressModel(
        endereco.id,
        endereco.client_id,
        endereco.street,
        ...
        endereco.city,
        endereco.state,
        endereco.primary_adress,
        endereco.zip_code,
      ).getAddress(),
    );

    const payment: Array<GetPaymentClient> = exists.payment_card.map(
      (payment) =>
        new PaymentModel(
          payment.id,
          payment.client_id,
  ...
  public updateClient = async (
    ...
    const newClient = new ClientModel(
      ...
      HashedPassword || exists.password,
      exists.cpf,
      dateOfBirth || exists.date_of_birth,
    ).updateClientDB();
  ...
  public deleteClient = async (
    input: deleteClientInputDTO,
  ): Promise<deleteClientOutputDTO> => {
    const { authorization } = input;
    const payload = this.tokenManager.getPayload(authorization.split(" ")[1]);
    if (payload === null) {
      throw new Unouthorized();
    ...
    return {
      message:
        "Sentimos muito que você tenha chegado a este ponto, Porem continuaremos aqui torcendo por você",
    };
  };

  //ADDRESS

  public createAdress = async (
    input: InsertAdressClientInputDTO,
  ): Promise<InsertAdressClientOutputDTO> => {
    const {
      authorization,
      street,
      houseNumber,
    ...
    const [adressAlReadyExists] =
      await this.clientDb.findAdressByStreet(street);
    if (adressAlReadyExists) {
      throw new BadRequest("'adress' - endereço ja registrado");
    }

    const id = this.idManager.generate();
    const complemento: string | null =
      complement === undefined ? null : complement;

    const Adress = new AddressModel(
      id,
      payload.id,
      street,
      houseNumber,
  ...
  public updateAdress = async (
    input: UpdateAdressClientInputDTO,
  ): Promise<UpdateAdressClientOutputDTO> => {
    const {
      authorization,
      id,
      street,
    ...
    if (
      ...
      throw new BadRequest(
        "'Atenção' - Informar ao menos um dado para atualização",
      );
    }
    ...
    const ADRESS = new AddressModel(
      adressAlReadyExists.id,
      adressAlReadyExists.client_id,
      street || adressAlReadyExists.street,
      houseNumber || adressAlReadyExists.house_number,
      complement || adressAlReadyExists.complement,
      district || adressAlReadyExists.district,
      city || adressAlReadyExists.city,
      state || adressAlReadyExists.state,
      primaryAdress || adressAlReadyExists.primary_adress,
      zipCode || adressAlReadyExists.zip_code,
    ).updateAdressDB();
  ...
  public deleteAdress = async (
    input: DeleteAdressClientInputDTO,
  ): Promise<DeleteAdressClientOutputDTO> => {
  ...
  public createPaymentCard = async (
    ...
    const cardAllReadyExistis =
      await this.clientDb.findPaymentMethodByNumberCard(numberCard);
  ...
  public deletePaymentCard = async (
    input: DeletePaymentInputDTO,
  ): Promise<DeletePaymentOutputDTO> => {
    const { authorization, id } = input;
    const payload = this.tokenManager.getPayload(authorization.split(" ")[1]);
    if (payload === null) {
      throw new Unouthorized();
    ...
    const Card = new PaymentModel(
      cardMethod.id,
      cardMethod.client_id,
      cardMethod.number_card,
