...
import {
  ...
  ProductsDeleteOutputDTO,
  ProductsInputDTO,
  ProductsOutputDTO,
  ProductsUpdateInputDTO,
  ProductsUpdateOutputDTO,
} from "../../dto/products";
...
export class ProductBusiness {
  ...
  public getProducts = async (): Promise<Array<GetProducts>> => {
    ...
    const products: Array<GetProducts> = lista.map((product) =>
      new ProductModel(
        product.id,
        product.title,
        product.description,
  ...
  public createProduct = async (
    input: ProductsInputDTO,
  ): Promise<ProductsOutputDTO> => {
    const { authorization, title, description, price, discount, imageUrl } =
      input;
  ...
  public updateProduct = async (
    input: ProductsUpdateInputDTO,
  ): Promise<ProductsUpdateOutputDTO> => {
    const { authorization, id, title, description, price, discount, imageUrl } =
      input;
    const payload = this.tokenManager.getPayload(authorization.split(" ")[1]);
  ...
  public deleteProduct = async (
    input: ProductsDeleteInputDTO,
  ): Promise<ProductsDeleteOutputDTO> => {
